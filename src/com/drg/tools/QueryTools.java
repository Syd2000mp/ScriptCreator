...
*/	
	...
	{
		//TODO: hacer este backup script dejando los datos en la cust_details_HIST para no generar tanta basura en producci�n
			...
			phoneMailData.forEach ((line,phoneMailD)->{
					...
					if (phonemail!=null) {
						...
						if ((rut!=null) && (!"".equalsIgnoreCase(rut))) {
							...
							try {
								 float mod = (float)line%4; //se pone 5 para pruebas y se deberia dejar 800
								 if (mod==0.0) {
		...
		if ((requestName!=null) && (!"".equalsIgnoreCase(requestName))) {
			...
			if (recordNum >0){
				...
				phoneMailData.forEach ((line,phoneMailD)->{
				    	...
				    	if (logger.isDebugEnabled()){
							logger.debug("Procesando la linea " + line);
						}
